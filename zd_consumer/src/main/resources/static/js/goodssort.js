...
function loadGoodsSort(jsonEntity){
  //方法级渲染
  table.render({
   elem: '#listTable'
   ,url: '/goodssort/listByPramas'
   ,width:1560
   ,height: 480
   ,page:true
   ,even:true
   ...
   ,cols: [[
     ...
     ,{field:'name', title: '分类名称',align:'center', width:160, sort: true}
  ...
  table.on('tool(list)', function(obj){
    ...
    if(obj.event === 'show'){
      ...
      layer.confirm('真的删除行么', function(index){
        ...
        czGoodsSort("/goodssort/delete",jsonEntity,obj);
        layer.close(index);
      });
      ...
      layer.open({
		...
		anim:4,
        shade:0.6,
  		content:$("#updGoodsSortDiv"),
  		success:function(){
        form.val("updGoodsSortForm", {
    "name": data.name,
        });

  		          //监听提交
			  form.on('submit(submitUpdGoodsSort)', function(FormData){
			    var jsonEntity = FormData.field
			    jsonEntity.id = data.id;
			    czGoodsSort("/goodssort/updT",jsonEntity,obj);
			    layer.closeAll();
			    return false;
			  });
  		}
    ...
    $("#delBatchGoodsSort").click(function(){
	   ...
	   if(data.length<1){
          layer.msg("请选择数据！！！",{"icon":3,anim:6,shade:0.6,"time":2000});
           ...
           layer.confirm("你确定要删除所选中的数据吗？？？", {icon: 7, title: '提示'}, function (index) {
               ...
               delManyGoodsSort("/goodssort/deletes", params);
               flush(currentPage);
               layer.close(index);
           });
       }
  ...
  $("#saveUI").click(function(){
	  layer.open({
	    ...
	    shade:0.6,
		  ...
		  form.on('submit(submitUpdGoodsSort)', function(FormData){
            var jsonEntity = FormData.field
		    czGoodsSort("/goodssort/saveT",jsonEntity);
		    layer.closeAll();
		    return false;
		  });
  		}
  ...
  //普通增删改操作
  function czGoodsSort(url,jsonEntity,obj){
	  $.ajax({
		   	type:"post",
		   	url:url,
		   	data:jsonEntity,
  ...
  //批量删除
  function delManyGoodsSort(url,ids){
	  $.ajax({
			type:"post",
			url:url,
			data:{'ids':ids},
