...
function loadGoodsDetail(jsonEntity){
  //方法级渲染
  table.render({
   elem: '#listTable'
   ,url: '/goodsdetail/listByPramas'
   ,width:1560
   ,height: 480
   ,page:true
   ,even:true
   ...
   ,cols: [[
     ...
     ,{field:'sortId', title: '分类编号',align:'center', width:160, sort: true}
     ,{field:'name', title: '商品名称',align:'center', width:160, sort: true}
     ,{field:'address', title: '产地',align:'center', width:160, sort: true}
     ,{field:'price', title: '单价',align:'center', width:160, sort: true}
     ,{field:'createDate', title: '生产日期',align:'center', width:160, sort: true}
     ,{field:'remaining', title: '剩余数量',align:'center', width:160, sort: true}
  ...
  table.on('tool(list)', function(obj){
    ...
    if(obj.event === 'show'){
      ...
      layer.confirm('真的删除行么', function(index){
        ...
        czGoodsDetail("/goodsdetail/delete",jsonEntity,obj);
        layer.close(index);
      });
      ...
      layer.open({
		...
		anim:4,
        shade:0.6,
  		content:$("#updGoodsDetailDiv"),
  		success:function(){
        form.val("updGoodsDetailForm", {
    "sortId": data.sortId,
    "name": data.name,
    ...
    "remaining": data.remaining,
  		          ...
  		          //监听提交
			  form.on('submit(submitUpdGoodsDetail)', function(FormData){
			    var jsonEntity = FormData.field
			    jsonEntity.id = data.id;
			    czGoodsDetail("/goodsdetail/updT",jsonEntity,obj);
			    layer.closeAll();
			    return false;
			  });
  		}
    ...
    $("#delBatchGoodsDetail").click(function(){
	   ...
	   if(data.length<1){
          layer.msg("请选择数据！！！",{"icon":3,anim:6,shade:0.6,"time":2000});
           ...
           layer.confirm("你确定要删除所选中的数据吗？？？", {icon: 7, title: '提示'}, function (index) {
               ...
               delManyGoodsDetail("/goodsdetail/deletes", params);
               flush(currentPage);
               layer.close(index);
           });
       }
  ...
  $("#saveUI").click(function(){
	  layer.open({
	    ...
	    shade:0.6,
		  ...
		  form.on('submit(submitUpdGoodsDetail)', function(FormData){
            var jsonEntity = FormData.field
		    czGoodsDetail("/goodsdetail/saveT",jsonEntity);
		    layer.closeAll();
		    return false;
		  });
  		}
  ...
  //普通增删改操作
  function czGoodsDetail(url,jsonEntity,obj){
	  $.ajax({
		   	type:"post",
		   	url:url,
		   	data:jsonEntity,
		   	success:function(data){
		   		if(data=="delSuccess"){
	   				...
	   				layer.msg("信息修改成功。。。",{"icon":6,anim:4,shade:0.6,"time":2000});
		   			 obj.update({
                                                                                                                sortId: jsonEntity.sortId,
                                                                                         name: jsonEntity.name,
                                                                                         address: jsonEntity.address,
                                                                                         price: jsonEntity.price,
                                                                                         createDate: jsonEntity.createDate,
                                                                                         remaining: jsonEntity.remaining,
                                           		   	      });
  ...
  //批量删除
  function delManyGoodsDetail(url,ids){
	  $.ajax({
			type:"post",
			url:url,
			data:{'ids':ids},
